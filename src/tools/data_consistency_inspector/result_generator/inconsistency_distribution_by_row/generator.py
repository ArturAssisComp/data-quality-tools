...
BAR_PLOT_SETTINGS = {
    'relative': {
        ...
        'fig_name': 'rel_inconsistency_distribution_by_row_overview.png',
        'suffix': '%'
    },
    'absolute': {
        'y_label': 'Row Count',
        'fig_name': 'abs_inconsistency_distribution_by_row_overview.png',
        'suffix': ''
    }
}
...
MAX_WIDTH = 25
X_LABEL = 'Inconsistency Per Row'
TITLE = 'Inconsistency  Distribution by Row Overview'

class InconsistencyDistributionByRowOverviewGenerator(BaseOverviewGenerator):
    _needed_snapshots:list[SnapshotType] = [SnapshotType.ROW_INCONSISTENCY_DISTRIBUTION_SNAPSHOT]
    ...
    def _handle_content(self, parsed_content_dict:dict[SnapshotType, Any], basedir_path:str, name_preffix:str=''):
        rowNullDistributionSnapshotContentModel = parsed_content_dict[SnapshotType.ROW_INCONSISTENCY_DISTRIBUTION_SNAPSHOT]
        try:
            try:
    ...
    # TODO refactor: extract this as a function (it is used in null distribution by row overview)
    def _generate_bar_plot(self, row_inconsistency_distribution_snapshot_content_model:RowInconsistencyDistributionSnapshotContent, basedir_path:str, name_preffix:str = '', relative:bool=False):
        content = row_inconsistency_distribution_snapshot_content_model.content
