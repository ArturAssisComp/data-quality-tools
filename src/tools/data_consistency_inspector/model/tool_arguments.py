from tools.base_tool_arguments import BaseToolArguments

class ToolArguments (BaseToolArguments):
    inconsistency_distribution_by_row_overview:bool = False
    ranked_inconsistency_count_by_column_overview:bool = False
    ranked_inconsistency_count_by_type_and_column_overview:bool = False
    inconsistent_frequent_pairs_overview:bool = False
    statistical_summary_overview:bool= False
