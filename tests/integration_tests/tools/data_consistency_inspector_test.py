...
def load_snapshots(base_dir:str):
    ...
    column_pair_inconsistency_pattern_snapshot = _load_snapshot(base_dir, get_snapshot_name(SnapshotType.COLUMN_PAIR_INCONSISTENCY_PATTERN_SNAPSHOT))
    return row_inconsistency_distribution_snapshot, column_inconsistency_count_by_type_snapshot, column_pair_inconsistency_pattern_snapshot
...
def execute_command(base_dir:str, midArguments = ''):
        os.chdir(base_dir)
        os.system(f'..\\..\\..\\venv\\Scripts\\data-quality-tools.exe {midArguments} dci -n0 -n1 -n2 -n3 -n4')
...
class TestDataConsistencyInspector:
    ...
    def test_strict_mode(self):
        ...
        row_inconsistency_distribution_snapshot, column_inconsistency_count_by_type_snapshot, column_pair_inconsistency_pattern_snapshot = execute_command(base_dir, '--doc ./documentation_dci.txt')
        ...
        assert column_pair_inconsistency_pattern_snapshot == {
            "type": "column_pair_inconsistency_pattern_snapshot", 
            "population":{"content": {'A':{'A':1}, "B": {'B':1}}}, 
            "samples":None,
            "state": "strict-mode", 
            "columns":['A', 'B']
        }
    ...
    def test_subset_mode(self):
        base_dir = os.path.dirname(os.path.abspath(__file__))
        row_inconsistency_distribution_snapshot, column_inconsistency_count_by_type_snapshot, column_pair_inconsistency_pattern_snapshot= execute_command(base_dir, '--doc ./documentation_dci_subset_mode.txt')
        assert row_inconsistency_distribution_snapshot == {
            "type": "row_inconsistency_distribution_snapshot", 
        ...
        assert column_pair_inconsistency_pattern_snapshot == {
            "type": "column_pair_inconsistency_pattern_snapshot", 
            "population":{"content": {'A':{'A':10, 'C':4}, "C": {'C':7, 'A':4}}}, 
            "samples":None,
            "state": "subset-mode", 
            "columns":['A', 'C']
        }
