...
class TestColumnInconsistencyCountByTypeSnapshot:
    ...
    initial_content_some_initial_values = {'A':{'A':2, 'B':1}, 'B':{'A':1}}
    '''
    ...
    @pytest.mark.parametrize('_, documentation, df_dict, initial_content, final_content', [
        ...
        ('2	strict_mode	only_zero	some_initial_values', strict_mode, df_dict_only_zero, initial_content_some_initial_values, {'A':{'A':2, 'B':1}, 'B':{'A':1}}),
        ('3	strict_mode	1 inconsistency	empty', strict_mode, df_dict_one_inconsistency, initial_content_empty, {'A':{'A':1}}),
        ('4	strict_mode	2 inconsistencies	some_initial_values', strict_mode, df_dict_two_inconsistencies, initial_content_some_initial_values, {'A':{'A':3, 'B':2}, 'B':{'B':1, 'A':2}}),
        ('5	subset_mode	only_zero	empty', subset_mode, df_dict_only_zero, initial_content_empty, dict()),
        ('6	subset_mode	1 inconsistency	some_initial_values', subset_mode, df_dict_one_inconsistency, initial_content_some_initial_values, {'A':{'A':3, 'B':1}, 'B':{'A':1}}),
        ('7	subset_mode	2 inconsistencies	empty', subset_mode, df_dict_two_inconsistencies, initial_content_empty, {'A':{'A':1}}),
        ('8	subset_mode	empty	some_initial_values', subset_mode, df_dict_empty, initial_content_some_initial_values, {'A':{'A':2, 'B':1}, 'B':{'A':1}}),
    ])
    def test_process_dataframe(self, _, documentation:Documentation, df_dict:dict, initial_content:dict, final_content:dict):
        ...
        snapshotBuilder = ColumnPairInconsistencyPatternSnapshot(documentation, logger=mock_logger, fileOperations=mock_file_operations)
